	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		if (searchButton != null) {			
			...
			public void handle(ActionEvent t) {
				...
				searchTextArea.setText(grep(regularExpression(textSearch.getText()),on(profilesToGrep)).toString());
				
			}

			private List<Profile> buildProfiles(List<SpyOneProfile> selectedProfile) {
				List<Profile> profilesToGrep = new ArrayList<Profile>();
				for (SpyOneProfile spyOneProfile : selectedProfile) {
					Profile profile = ProfileBuilder.newBuilder().name(spyOneProfile.getProfileName()).filePath(spyOneProfile.getFilePath()).onLocalhost().build();
